...
row and are managed as a unit. Rows are read-only by default — name(s) and URL
shown as plain text with an *Edit* button — so the listing stays easy to scan;
the inputs and the *Save*/*Delete* controls appear only for the row you choose
to edit. All writes preserve your config's existing comments and formatting, are
atomic (a crash never truncates the file), and take effect on the next request.
...
- **Edit**: press a row's *Edit* button to open it for editing in place. Change
  the alias list and/or the URL and press *Save* — adding a name to the list
  creates a new alias for that entry, removing one deletes that alias. Naming a
  command that already belongs to a *different* entry is refused so you can't
  silently clobber another shortcut. (A removed alias loses any TOML comment on
  its line; aliases that stay keep theirs, and comments elsewhere are untouched.)
  *Cancel* leaves edit mode without changing anything.
- **Delete**: while editing a row, the *Delete* button removes the whole entry —
  every alias that points at its URL.
