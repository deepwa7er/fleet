...
deploy/
  ...
  lighthouse.target    discovery target (members are the monitored units)
  provision.sh         infra: user, config, target, polkit grant, unit
deploy.toml      tugboat manifest — routine code/asset deploys
...
a non-root caller the action is authorized by **polkit**. `provision.sh` installs
...
The polkit grant is regenerated by `provision.sh` from the target's members (plus
...
**controllable** only after the next `deploy/provision.sh` (which is when the
grant is written — Lighthouse runs unprivileged and can't edit polkit rules
itself).
...
**Provision the infra** (service user, config, discovery target, polkit grant,
systemd unit) — first-time setup, and again whenever you enroll a new service in
`lighthouse.target` and want lighthouse to control it:

```sh
deploy/provision.sh
```

It auto-detects the Tailscale IP and writes it into `bind`.

**Ship code/assets** with [tugboat](https://github.com/deepwa7er/tugboat) — builds
the frontend (bun) and a static musl backend locally, ships the binary + web
assets, swaps them in atomically, restarts, health-checks, and rolls back on
failure:
...
Open `http://<tailscale-ip>:8080` from any device on your tailnet.
