...
# under the unit's NoNewPrivileges=true. The allowed-unit list is regenerated
# from the installed config on every deploy, so adding a service for control
# means re-running this script (viewing only needs a config edit + restart).
UNIT_LIST="$(grep -oP '^\s*unit\s*=\s*"\K[^"]+' /etc/lighthouse/config.toml)"
UNIT_ARRAY="$(printf '%s\n' "$UNIT_LIST" | sed 's/^/        "/; s/$/",/')"
