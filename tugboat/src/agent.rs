...
//!
//! The binary is cross-compiled for each target. No health-check/rollback/ledger
//! — these are trivially replaceable user daemons, not the VPS's load-bearing
//! services.
...
pub struct AgentManifest {
    ...
    /// the output path the binary must end up at; `{goos}` and `{goarch}` expand
...
pub struct Target {
    ...
    /// Target OS for the cross-compiled build (`{goos}`).
    pub goos: String,
    /// Target architecture for the cross-compiled build (`{goarch}`).
    pub goarch: String,
...
impl Target {
    /// The shell command that restarts the daemon on its machine.
...
fn deploy_target(dir: &Path, manifest: &AgentManifest, target: &Target, dry_run: bool) -> Result<()> {
    ...
    match &target.ssh {
        Some(host) => install_remote(host, &out, &target.dest, &restart),
        None => install_local(&out, &target.dest, &restart),
    }
}
...
fn load(path: &Path) -> Result<AgentManifest> {
    ...
    for t in &manifest.targets {
        ...
        match (&t.launchd, &t.systemd_user) {
            ...
            (None, None) => {
                bail!("agent target `{}`: set `launchd` or `systemd_user`", t.name)
            }
            _ => {}
        }
...
mod tests {
    ...
    fn load_text(text: &str) -> Result<AgentManifest> {
    ...
    #[test]
    fn loads_launchd_and_systemd_daemon_targets() {
        let manifest = load_text(
            r#"
name = "clipd"
build = "GOOS={goos} GOARCH={goarch} go build -o {out} ./cmd/clipd"
...
"#,
        ...
        assert_eq!(
            manifest.targets[0].restart_cmd(),
            "launchctl kickstart -k gui/$(id -u)/'com.example.clipd'"
        );
        assert_eq!(
            manifest.targets[1].restart_cmd(),
    ...
    fn rejects_a_target_without_a_restart_manager() {
        let error = load_text(
            r#"
name = "tool"
build = "GOOS={goos} GOARCH={goarch} go build -o {out} ./cmd/tool"
...
"#,
    ...
    #[test]
    fn rejects_a_target_without_a_cross_compilation_platform() {
        let error = load_text(
            r#"
name = "clipd"
build = "GOOS={goos} GOARCH={goarch} go build -o {out} ./cmd/clipd"
...
"#,
        ...
        assert!(format!("{error:#}").contains("missing field `goos`"));
    }
}
